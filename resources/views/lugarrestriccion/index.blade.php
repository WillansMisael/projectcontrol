        ...
        <div class="col-md-3 col-sm-12">
            ...
            @csrf
                <div class="form-group">
                    <label for="exampleFormControlInput1">Lugares</label>
...
<script>
    ...
    $(document).ready(function(){
        var lugarrestriccion = $('#tabla-lugar-restriccion').DataTable({
            ...
            serverSide:true,
            ajax:{
                url: "{{ route('lugarrestriccion.index') }}",
            },
            columns:[
                {data: 'id'},
                {data: 'lugar'},
...
<script>
    ...
    $('#registro-lugar-restriccion').submit(function(e){
        e.preventDefault();
        var lugar = $('#txtLugar option:selected').val();
        var restriccion = $('#txtRestriccion option:selected').val();
        var _token = $("input[name=_token]").val();

        $.ajax({
            url: "{{ route('lugarrestriccion.registrar') }}",
            type: "POST",
            data:{
                lugar: lugar,
...
//eliminar un registro
    ...
    $('#btnEliminar').click(function(){
        $.ajax({
            url: "lugarrestriccion/eliminar/"+id,
            beforeSend:function(){
