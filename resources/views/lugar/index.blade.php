    ...
    $('#registro-lugar').submit(function(e){
        ...
        $.ajax({
            ...
            success:function(response){
                if(response){
                    $('#registro-lugar')[0].reset();
                    toastr.success('El registro se ingreso correctamente.','Nuevo Registro',{timeOut:3000});
                    $('#tabla-lugar').DataTable().ajax.reload();
                }
            }
        });
    });
...
// editar un registro
    function editarLugar(id){
        //recuperamos datos
        $.get('lugar/editar/'+id, function(lugar){
            //asignar los datos recuperados a la ventana modal\
            $('#txtId2').val(lugar[0].id_lugar);
            $('#txtNombre2').val(lugar[0].nombre);
            $('#txtDescripcion2').val(lugar[0].descripcion);
            if(lugar[0].estado == "Activo"){
                $('input[name=rbEstado2][value="Activo"]').prop('checked','checked');
            }
            if(lugar[0].estado == "Inactivo"){
                $('input[name=rbEstado2][value="Inactivo"]').prop('checked',true);
            }
            $("input[name=_token]").val();
            $('#lugar_edit_modal').modal('toggle');
        });
    }
</script>
<script>
